...
class webscraper():
    def __init__(self, wait = 160):
        self.dictionary = { "Lecture" : 0, "Laboratory" : 1, "Recitation" : 2, "Recitation (Optional)" : 3, "Internship" : 4, "Seminar" : 5, 
        ...
        self.dictionary_array = ["Lecture", "Laboratory", "Recitation", "Recitation (Optional)", "Internship", "Seminar", "Continuance", "Project", "Teaching Assistant", "Independent Study", "Thesis", "Research", "Laboratory (Optional)", "Discussion", "Thesis Research", "Exam", "Clinical", "Rotation", 
    ...
    ##initialize course_list and tr_list
    def init_lists(self, bsoup, course_list, tr_list):
        div_list = []
        #all sections of a section_type
        tbody_list = []
        #recitations
        tables_list = []
        #finding course group
        div_list = bsoup.find_all('div','tfp_accordion_row cls-show-js')
        ...
        for d in div_list:
            ...
            #extract section types (every tbody is a section type)
        ...
        #TODO ACCOUNT FOR ALL TYPES OF SECTION TYPES AND CHECK
        ...
        for course in tbody_list:
            ...
            for tbody in course:
                ...
                index_to_use = self.dictionary[section_type]
    ...
    ##populate master_list with course names and course ID's
    ...
    def input_section_name(self, tr, single_section):
        td = tr.find('td')
    ...
    #takes a string formatted like We, ThXX:XXAM - XX:XXPM
    ...
    def scrape_web(self):
        ...
        #finding course name and course ID
        course_list = []
        #lectures&recits
        tr_list = []
        #extract info from every course
        self.init_lists(bsoup, course_list, tr_list)

        #extracting lectures
        size = len(course_list)
        master_list = []
        for x in range(0, size):
            #master_list.append(["course_name",[[["lec section name", "teacher", "time", "location"],...],
            #[["lab sec name",..],...],...]])
            master_list.append(["course_name","courseID",[]])
        #iterate through each course
        for x in range(0,size):
            #put course name and course ID in master_list
            self.input_course_name_ID_master(master_list, course_list, x)
            #iterate through section_types (there are 9)
            for y in range (0,self.total_section_types):
                #iterate through sections
                sections = []
                #gather info about a particular section
                for tr in tr_list[x][y]:
                    #single_section : ["section name", "time(day and time)", "location", "city", "teacher"]
                    single_section = []
                    section_name = str(self.input_section_name(tr,single_section))
                    #optional recit
                    if y == 3:
                    ...
                    div_tfploc = tr.find_all('div','tfp-loc')
                    options = []
                    for divtl in div_tfploc:
                        ...
                        #except Exception as e:
                            #print ("error at input_time in scrape_web() because of:")
                            #print e
                            #print "div_time_loc"
                            #print div_time_loc
                            #print "course"
    ...
    def write_json(self, master_list):
        ...
        for course_num in range(0, courses_amnt):
            ...
            for sec_type_num in range(0, self.total_section_types):
                section_type = self.dictionary_array[sec_type_num]
                    ...
                    for sec_num in range(0, section_amnt):
                        ...
                        for subsec_num in range(0, subsec_size):
                            ...
                            for subsec_time in master_list[course_num][2][sec_type_num][sec_num][1][subsec_num][0]:
                                times_list.append(str(subsec_time))
