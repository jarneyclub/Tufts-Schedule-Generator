...
class webscraper():
    ...
    def init_tr_list ( self, tables_list, tbody_list ) :
        ...
        for course in tables_list:
            ...
            for table in course:
                ...
                if len(table.find_all('caption')) > 0:
    ...
    def write_json(self, master_list):
        ...
        for course_num in range(0, courses_amnt):
            # DEFINE course name and course ID
            course_name = master_list[course_num][0] #NEW
            course_id = master_list[course_num][1] #NEW
            ...
            for sec_type_num in range(0, self.total_section_types):
                ...
                if len(master_list[course_num][2][sec_type_num]) > 0:
                    ...
                    for sec_num in range(0, section_amnt):
                        ...
                        # make course name tuple
                        course_name_tuple = ("course_name", course_name)
            ...
            # END OF (section type iteration)
            # check if there already exists an entry with matching course_id in dict_all_courses
            try: #NEW
                if (dict_all_courses[course_id]):#NEW
                    # there already exists an entry in dict_all_courses with this course_id (THIS MEANS THERE'S MORE THAN ONE COURSE 
                    # ASSOCIATED WITH THIS COURSE ID. e.g. EN-0001 )
                    dict_all_courses[course_id]['multifunction'] = True #NEW
            except KeyError: #NEW
                # an entry with the current course_id does not yet exist in dict_all_courses
                sec_type_header = ("section_types", available_sec_types)
                sec_type_list.append(sec_type_header)
            #sec_type_header = ("section_types", available_sec_types)
            #sec_type_list.append(sec_type_header)
            all_courses.append((course_id, dict(sec_type_list)))
            dict_all_courses = dict(all_courses)  # NEW 
        dict_file['courses'] = dict_all_courses # NEW (modified)
